...
def get_arg_parser():
    ...
    parser.add_argument("--epoch", type=int, default=639)
    ...
    ### training
    parser.add_argument("--depth_loss", type=str, default="l1", help="l1, bce, mse, huber, cce")
    parser.add_argument("--depth_grad_loss", type=str, default="l1", help="l1, bce, mse, huber, cce")
    parser.add_argument("--intensity_loss", type=str, default="mse", help="l1, bce, mse, huber, cce")
    parser.add_argument("--raydrop_loss", type=str, default="mse", help="l1, bce, mse, huber, cce")
    parser.add_argument("--label_loss", type=str, default="cce", help="l1, bce, mse, huber, cce")
    ...
    parser.add_argument("--alpha_l", type=float, default=0.01)
...
def main():
    ...
    labels = torch.tensor([8,7,11,26,21,100]).to(device)
    ...
    # gt_dict = {int(label.item()): one_hot_encoded[i] for i, label in enumerate(labels)}
    gt_dict = OrderedDict()
    ...
    if opt.test or opt.test_eval or opt.refine:
        ...
        if test_loader.has_gt and not opt.test:
            trainer.evaluate(test_loader, gt_dict)

        trainer.test(test_loader,gt_dict, write_video=False)
        ...
        trainer.train(train_loader, valid_loader, refine_loader, max_epoch, gt_dict)
        ...
        if test_loader.has_gt:
            trainer.evaluate(test_loader, gt_dict)  # evaluate metrics

        trainer.test(test_loader, gt_dict, write_video=False)  # save final results
