...
from utils.convert import lidar_to_pano_with_intensities, pano_to_lidar_with_intensities, pano_to_lidar, pano_to_lidar_with_intensities_label
...
class KITTI360Dataset(BaseDataset):
    ...
    def __post_init__(self):
        ...
        for f in tqdm.tqdm(frames, desc=f"Loading {self.split} data"):
            ...
            # pc = pano_to_lidar_with_intensities_label(pano=pc[:, :, 2], intensities=pc[:,:,1], labels= pc[:, :, 3], lidar_K=self.fov_lidar)
            # print(pc.shape)
            ray_drop = np.where(pc.reshape(-1, 4)[:, 2] == 0.0, 0.0, 1.0).reshape(
