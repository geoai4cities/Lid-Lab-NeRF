...
def main():
    ...
    # Define main output directories for lidar, image, and semantic map data.
    range_view_dir = kitti_360_parent_dir / "train"
    # lidar_dir = range_view_dir / "lidar"
    # image_dir = range_view_dir / "image"
    # semantic_map_dir = range_view_dir / "semantic_map"

    # Get lidar paths (range view, not raw data), image paths, and semantic map paths.
    range_view_paths = [
        range_view_dir / "{:010d}.npy".format(int(frame_id)) for frame_id in frame_ids
    ]
    # image_paths = [
    #     image_dir / "{:010d}.png".format(int(frame_id)) for frame_id in frame_ids
    # ]
    # semantic_map_paths = [
    #     semantic_map_dir / "{:010d}.png".format(int(frame_id)) for frame_id in frame_ids
    # ]
    ...
    for split, indices in split_to_all_indices.items():
        ...
        json_dict = {
            ...
            "semantic_data_static": args.static_semantic_data_path,
            "dynamic_data_static": args.dynamic_semantic_data_path,
            "frames": [
                {
                    "frame_id": id,
                    "lidar_file_path": str(lidar_path.relative_to(kitti_360_parent_dir)),
                    "lidar2world": lidar_world.tolist(),
                }
                for (
                    id,
                    lidar_path,
