...
class LiDAR_Renderer(nn.Module):
    ...
    def run(
        ...
        **kwargs
        ...
        label = torch.sum(weights.unsqueeze(-1) * label, dim=-2)
        
        image = image.view(*prefix, out_lidar_dim)
        depth = depth.view(*prefix)
        # label = label.view(*prefix, 21)
    ...
    def render(
        ...
        **kwargs
        ...
        if staged:
            out_lidar_dim = self.out_lidar_dim
            res_keys = ["depth_lidar", "image_lidar", "label"]
            depth = torch.empty((B, N), device=device)
            image = torch.empty((B, N, out_lidar_dim), device=device)
            label = torch.empty((B, N, 6), device=device)
            ...
            for b in range(B):
                ...
                while head < N:
                    ...
                    label[b : b + 1, head:tail] = results_[res_keys[2]]
                    head += max_ray_batch

            results = {}
            results[res_keys[0]] = depth
            results[res_keys[1]] = image
            results[res_keys[2]] = label
