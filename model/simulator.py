...
from utils.convert import pano_to_lidar, pano_to_lidar_with_intensities_label
...
class Simulator(object):
    ...
    def render(self, rays_o_lidar, rays_d_lidar, times_lidar, save_pc=True, save_img=True, save_video=True):
        ...
        for i in tqdm.tqdm(range(B)):
            ...
            pred_rgb_lidar = outputs_lidar["image_lidar"].reshape(-1, self.H_lidar, self.W_lidar, 3)
            ...
            pred_label_visual = pred_label_visual.reshape(-1, self.H_lidar, self.W_lidar) + 1
            ...
            pred_label_visual = pred_label_visual[0].detach().cpu().numpy()
            unique_labels = np.unique(pred_label_visual)
            pred_lidar = pano_to_lidar_with_intensities_label(pred_depth/self.opt.scale, pred_intensity, pred_label_visual,
                                                                 self.opt.fov_lidar)
            ...
            if save_pc:
                ...
                np.savetxt(save_path[:-3]+"txt", pred_lidar)
            ...
            if save_img:
                ...
                img_colored_label[img_label == 0] = [0, 0, 0]
