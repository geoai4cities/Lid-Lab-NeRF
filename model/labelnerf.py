...
# Apache License 2.0
# 
# Modified by Shrestha Srivastava (2025)
# Significant code changes and new features added
# ==============================================================================
import torch
...
class Labelnerf(LiDAR_Renderer):
    def __init__(
        ...
        num_layers_lidar=3,
        hidden_dim_lidar=64,
        out_lidar_dim=3, # 3 since labels are included along with raydrop and intensity
        num_frames=51,
        bound=1,
        **kwargs,
    ):
        ...
        #my addition a network for label prediction
        self.label_net = tcnn.Network(
            n_input_dims=self.view_encoder.n_output_dims + geo_feat_dim,
            n_output_dims=6,
            network_config={
                "otype": "FullyFusedMLP",
    ...
    def attribute(self, x, d, mask=None, geo_feat=None, **kwargs):
        ...
        if mask is not None:
            ...
            label_output = torch.zeros(
                mask.shape[0], 6, dtype=x.dtype, device=x.device   #6 stands for labels
            )  # [N, 3]
        ...
        label = F.softmax(self.label_net(torch.cat([d, geo_feat], dim=-1)), dim=1)
        # one_hot_output = torch.zeros_like(label)
        # max_indices = torch.argmax(label,dim = 1)
        # one_hot_output[torch.arange(label.size(0)), max_indices] = 1
        label_max, label_index = torch.max(label, dim = 1)
        
        # h = torch.cat([raydrop, intensity], dim=-1)
        h = torch.cat([raydrop, intensity, label_index.unsqueeze(1)], dim=-1)
        if mask is not None:
            output[mask] = h.to(output.dtype)  # fp16 --> fp32
            label_output[mask] = label.to(label_output.dtype)
        else:
            output = h
    ...
    def get_params(self, lr):
        params = [
            ...
            {"params": self.label_net.parameters(), "lr": 0.1 * lr},
        ]
