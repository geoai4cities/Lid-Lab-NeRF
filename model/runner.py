...
class Trainer(object):
    def __init__(
        ...
        self.model = model
        # THIS FOR FEATURE LOSS
        # self.feature_extractor = vgg16(pretrained=True).features.eval().cuda()
        # for param in self.feature_extractor.parameters():
        #     param.requires_grad = False  # Freeze feature extractor
        # self.feature_weights = [1 / (2 ** (4 - i)) for i in range(4)]  # Example weights
        self.feature_extractor = VGGLoss(self.device)
    ...
    def train_step(self, data, gt_dict):
        ...
        label_mask = gt_raydrop.bool()
        gt_intensity = images_lidar[:, :, 1] * gt_raydrop
        gt_depth = images_lidar[:, :, 2] * gt_raydrop
        gt_label = images_lidar[:, :, 3].long()[label_mask] # removed gt_raydrop
        
        gt_one_hot = torch.stack([gt_dict[int(one_hot.item())] for one_hot in gt_label.squeeze(0)])
        ...
        pred_raydrop = outputs_lidar["image_lidar"][:, :, 0] # shape is [1,1024]
        pred_intensity = outputs_lidar["image_lidar"][:, :, 1] * gt_raydrop
        # pred_label = outputs_lidar["image_lidar"][:, :, 2] * gt_raydrop
        pred_label = outputs_lidar["label"].unsqueeze(0)[label_mask] # applying label mask as right now we are not concerned with raydrop i.e, 0 that is in our output
        ...
        lidar_loss = (
            ...
            + self.opt.alpha_l * self.criterion['label'](pred_label, gt_one_hot)
        )
    ...
    def eval_step(self, data, gt_dict):
        pred_intensity = None
        pred_depth = None
        pred_raydrop = None
        gt_intensity = None
        ...
        gt_raydrop = images_lidar[:, :, :, 0]
        gt_label_mask = gt_raydrop.view(1,-1).bool()
        
        gt_intensity = images_lidar[:, :, :, 1] * gt_raydrop
        gt_depth = images_lidar[:, :, :, 2] * gt_raydrop
        gt_label = images_lidar[:, :, :, 3].view(1,-1)[gt_label_mask]
        # gt_label = images_lidar[:, :, 3].long() * gt_raydrop # removed gt_raydrop
        gt_one_hot = torch.stack([gt_dict[int(one_hot.item())] for one_hot in gt_label.squeeze(0)])
        ...
        pred_rgb_lidar = outputs_lidar["image_lidar"].reshape(-1, H_lidar, W_lidar, 3)
        pred_raydrop = pred_rgb_lidar[:, :, :, 0]
        pred_intensity = pred_rgb_lidar[:, :, :, 1]
        # pred_label_visual = pred_rgb_lidar[:, :, :, 2]
        
        pred_label = outputs_lidar['label']
        
        _,pred_label_visual = torch.max(pred_label, dim=2)
        
        pred_label_visual = pred_label_visual.reshape(-1, H_lidar, W_lidar)
        ...
        if self.opt.raydrop_loss == 'bce':
            pred_raydrop = F.sigmoid(pred_raydrop)
        if self.use_refine:
            pred_raydrop = torch.cat([pred_raydrop, pred_intensity, pred_depth, pred_label_visual+1], dim=0).unsqueeze(0)
            pred_raydrop = self.model.unet(pred_raydrop).squeeze(0)
        raydrop_mask = torch.where(pred_raydrop > 0.5, 1, 0)
        # print("PRED LABEL ater mask")
        # print(pred_label.squeeze(0)[label_mask].shape)
        ...
        lidar_loss = (
            ...
            + self.opt.alpha_l * self.criterion['label'](pred_label[gt_label_mask], gt_one_hot).mean()
        )
    ...
    def test_step(self, data, perturb=False):
        ...
        pred_raydrop = pred_rgb_lidar[:, :, :, 0]
        pred_intensity = pred_rgb_lidar[:, :, :, 1]
        # pred_label = pred_rgb_lidar[:, :, :, 2]
        pred_label = outputs_lidar['label']
        _,pred_label_visual = torch.max(pred_label, dim=2)
        pred_label_visual = pred_label_visual.reshape(-1, H_lidar, W_lidar) + 1 # we are adding 1 so that class 0 becomes 1, since 0 in pred label and ground truth has different meaning,
                                                                                # in pred_label 0 means first label, but in gt_label it means raydrop or missing data generated when point clouds are converted 
                                                                                # to rangeview images.
        pred_depth = outputs_lidar["depth_lidar"].reshape(-1, H_lidar, W_lidar)
        ...
        if self.opt.raydrop_loss == 'bce':
            pred_raydrop = F.sigmoid(pred_raydrop)
        if self.use_refine:
            pred_raydrop = torch.cat([pred_raydrop, pred_intensity, pred_depth, pred_label_visual], dim=0).unsqueeze(0)
            pred_raydrop = self.model.unet(pred_raydrop).squeeze(0)
        raydrop_mask = torch.where(pred_raydrop > 0.5, 1, 0)
        if self.opt.alpha_r > 0:
            pred_intensity = pred_intensity * raydrop_mask
            pred_depth = pred_depth * raydrop_mask
            pred_label_visual = pred_label_visual * raydrop_mask
            
        return pred_raydrop, pred_intensity, pred_depth, pred_label_visual

    ### ------------------------------

    def train_one_epoch(self, loader, gt_dict):
    ...
    def evaluate_one_epoch(self, loader, gt_dict, name=None):
            ...
            for data in loader:
                ...
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    (
                        ...
                        gt_one_hot,
                        loss,
                    ) = self.eval_step(data, gt_dict)

                preds_mask = torch.where(preds_raydrop > 0.5, 1, 0)
                labels_mask = gt_raydrop.view(1,-1).bool()
                ...
                for i, metric in enumerate(self.lidar_metrics):
                    if i == 0:  # hard code
                        ...
                        print(metric)
                        metric.update(pred_label[labels_mask], gt_one_hot)
                ...
                pred_label_visual = pred_label_visual[0].detach().cpu().numpy() + 1 # we do thsis so that when pred_mask is multiplied later on, it makes sense. since 0 is already a label, and when pred mask is multiplied
                                                                                    # it makes certain label as 0 as it shows raydrop, so if we dont increase labels by 1 it means that certain labels are being converted to 
                                                                                    # label 1 (i.e, 0) which is wrong. 0 must represent raydrop or missing data and not labels.
                unique_labels = np.unique(pred_label_visual)
                print(unique_labels)
                img_label = (pred_label_visual / (len(unique_labels) - 1) * 255).astype(np.uint8)
                img_colored_label = cv2.applyColorMap(img_label, cv2.COLORMAP_JET)
                
                # label_to_index = {label: idx for idx, label in enumerate(unique_labels)}
                img_label = (pred_label_visual * 255).astype(np.uint8)
                img_label = cv2.applyColorMap(img_label, 1)
                ...
                img_label_masked = ((pred_label_visual) * preds_mask)
                img_label_masked = (img_label_masked / (len(unique_labels) - 1) * 255).astype(np.uint8)
                img_masked_colored_label = cv2.applyColorMap(img_label_masked, cv2.COLORMAP_JET)
                img_masked_colored_label[img_label_masked == 0] = [0, 0, 0]  # BGR format for black, this is done to show that 0 means missing data and not a class or label hence the black colour

                img_pred = cv2.vconcat([img_raydrop, img_intensity, img_depth, img_colored_label,
                                        img_raydrop_masked, img_intensity_masked, img_depth_masked, 
                                        img_masked_colored_label])
    ...
    def train(self, train_loader, valid_loader, refine_loader, max_epochs, gt_dict):
        if self.use_tensorboardX:
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            ...
            self.train_one_epoch(train_loader, gt_dict)
            ...
            if self.epoch % self.eval_interval == 0:
                ...
                self.evaluate_one_epoch(valid_loader, gt_dict)
                
        self.refine(refine_loader, gt_dict)

        if self.use_tensorboardX:
            self.writer.close()

    def evaluate(self, loader, gt_dict, name=None, refine=True):
        self.use_refine = refine
        self.use_tensorboardX, use_tensorboardX = False, self.use_tensorboardX
        self.evaluate_one_epoch(loader, gt_dict, name)
        self.use_tensorboardX = use_tensorboardX

    def test(self, loader, gt_dict, save_path=None, name=None, write_video=True, refine=True):
        ...
        with torch.no_grad():
            for i, data in enumerate(loader):
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    preds_raydrop, preds_intensity, preds_depth, pred_label_visual = self.test_step(data) 
                ...
                pred_depth = preds_depth[0].detach().cpu().numpy()
                print(max(pred_label_visual))
                # for i,j in enumerate(pred_label_visual):
                #     pred_label_visual[i] = list(gt_dict.keys())[j-1] # j-1 because pred label is increased by one but on querying gt_dict 0 means first class and and not raydrop or missing data, hence we make it -1
                
                pred_label_visual = pred_label_visual[0].detach().cpu().numpy() * pred_raydrop
                
                # pred_lidar = pano_to_lidar(
                #     pred_depth / self.opt.scale, loader._data.intrinsics_lidar
                # )
                pred_lidar= pano_to_lidar_with_intensities_label(pred_depth, pred_intensity, pred_label_visual,
    ...
    # def compute_feature_loss(self, pred, gt):
    #     """
    #     Compute the feature-level loss (L_feat) between pred and gt.
    #     :param pred: Predicted input [B, C, H, W]
    #     :param gt: Ground-truth input [B, C, H, W]
    #     :return: Feature loss scalar
    #     """
    #     feature_loss = 0.0
    #     pred_features = pred
    #     gt_features = gt

    #     for level, weight in zip(self.feature_extractor, self.feature_weights):
    #         # Extract features at this level
    #         pred_features = level(pred_features)
    #         gt_features = level(gt_features)

    #         # Compute L1 loss for this level
    #         feature_loss += weight * F.l1_loss(pred_features, gt_features)

    #     return feature_loss
    


    def refine(self, loader, gt_dict):
        w_feat = 0.5
        if self.ema is not None:
        ...
        for i, data in enumerate(loader):
            ...
            gt_vgg = data["images_lidar"][:, :, :, 1:4].permute(0,3,1,2) # considering label for feature alignment loss
            ...
            pred_raydrop = pred_rgb_lidar[:, :, :, 0]
            pred_intensity = pred_rgb_lidar[:, :, :, 1]
            pred_label = pred_rgb_lidar[:, :, :, 2] + 1
            # pred_label = outputs_lidar['label']
            # _,pred_label_visual = torch.max(pred_label, dim=2)
            # pred_label_visual = pred_label_visual+1 # adding one because 0 label has different meaning here, it refers to first label
                                                    # whereas the 0 in ground truth means no record or no label (because of raydrop)
            
            # pred_label_visual = pred_label_visual.reshape(-1, H_lidar, W_lidar)
            pred_depth = outputs_lidar["depth_lidar"].reshape(-1, H_lidar, W_lidar)

            raydrop_input = torch.cat([pred_raydrop, pred_intensity, pred_depth, pred_label], dim=0).unsqueeze(0)
            vgg_input = torch.cat([pred_intensity, pred_depth, pred_label], dim=0).unsqueeze(0)
            
            raydrop_input_list.append(raydrop_input)
            raydrop_gt_list.append(gt_raydrop)
            
            vgg_input_list.append(vgg_input)
            vgg_gt_list.append(gt_vgg)
        ...
        vgg_input = torch.cat(vgg_input_list, dim=0).cuda().float().contiguous()
        vgg_gt = torch.cat(vgg_gt_list, dim = 0).cuda().float().contiguous()
        ...
        for i in range(refine_epoch):
            ...
            raydrop_refine_mask = torch.where(raydrop_refine > 0.5, 1, 0)
            
            bce_loss = self.bce_fn(raydrop_refine, gt)
            # feature_loss = self.compute_feature_loss(vgg_input*raydrop_refine_mask, vgg_gt)
            feature_loss= self.feature_extractor(vgg_input*raydrop_refine_mask, vgg_gt )
            # print(feature_loss)
            total_loss = bce_loss + w_feat * feature_loss
            ...
            if i % 50 == 0:
                log_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                self.log(f"[{log_time}] iter:{i}, lr:{optimizer.param_groups[0]['lr']:.6f}, "
                         f"BCE Loss:{bce_loss.item()}, Feature Loss:{feature_loss.item()}, Total Loss:{total_loss.item()}")
